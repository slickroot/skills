...
Build each slice with TDD when it makes sense: write a failing test first, write just enough code to make it pass, then clean up. Commit after each green slice. Tests should assert on behavior, not on implementation details — never hardcode a literal that duplicates a constant in the code under test (e.g. `expect(retries).toBe(3)`); assert against the named constant or the observable outcome instead, so changing the constant later doesn't break the test for no reason. Avoid comments — code should be self-explanatory through naming and structure; a comment describing what code does goes stale the moment the code changes. Only write one for something the code can't say for itself: a bug workaround, a non-obvious constraint, or a genuinely weird edge case. Don't touch README.md or other docs — that's out of scope for a slice unless the spec explicitly asks for a docs change. When all slices are done and the tests pass, push the branch and open a pull request with `gh pr create` that links back to the issue. Add a QA section for the reviewer to run manual tests.
