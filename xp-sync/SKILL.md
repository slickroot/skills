...
1. ACTIVE STORIES -> CREATE INDIVIDUAL SPECS: For every story assigned to the current immediate release/iteration, create a new github issue for it and create a new, empty markdown file under `docs/specs/00X-[story-slug].md`. Populate it ONLY with the User Story text and an empty template header for the upcoming technical design phase. Make sure to include the github issue URL in the local markdown.
